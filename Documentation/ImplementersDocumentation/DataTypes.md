...
テーブルのカラムは、スキーマのバージョンと、`xs:restriction` 制約に必要な`xs:base` タイプに応じて、タイプの有効性を決定する。

| <nobr>データ型</nobr>     | <nobr>Ifc2x3</nobr> | <nobr>Ifc4</nobr> | <nobr>Ifc4x3</nobr> | <nobr>制限</nobr>塩基タイプ     |
| --------------------------------------------- | ------ | ------ | ------ | --------------------- |
...
## XMLの基本型
`xs:restriction` の`base` 属性に有効なXML基本型のリストと、文字列表現の妥当性をチェックするための関連正規表現は以下の通りである：

| <nobr>ベース</nobr>タイプ | 文字列正規表現<nobr>制約</nobr>  |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------- |
