...
テーブルのカラムは、スキーマのバージョンと、`xs:restriction`制約に必要な`xs:base`型に応じて、型の妥当性を決定する。

| データ型 | Ifc2x3 | Ifc4 | Ifc4x3 | 制限の基準型 |
| --------------------------------------------- | :-----: | :-----: | :-----: | --------------------- |
...
[IFCSTRIPPEDOPTIONALは](https://standards.buildingsmart.org/IFC/RELEASE/IFC4_3/HTML/lexical/IfcStrippedOptional.htm)決してインスタンス化されるべきではない特殊なデータ型ですが、スキーマ耐性のためにここに記載されていることに注意してください。

## XMLの基本型
`xs:restriction`、`base`属性に有効なXMLの基準型のリスト、文字列表現の妥当性をチェックする関連正規表現、`xs:restriction`内部で許可されるノードは以下の通り：

| ベースタイプ | 値文字列正規表現制約 | 注釈 | パターン | 列挙 | 最小長 | 最大長 | 長さ | 最小排他 | 最大排他 | 最小包含 | 最大包含 |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------- | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: |
...
- 数字を指定する場合: 小数の区切り文字としてドットを使用し、千単位の区切り文字は使用しないでください (例:`4.2` は有効ですが、`1.234,5` は無効です)。科学表記も可能です（例：`1000` を表すには`1e3` ）。
- ブーリアンを指定する場合：有効な値は`true` または`false` 、`0` 、`1` 。

## 備考
このドキュメントは[IDS Audit Tool のリポジトリから](https://github.com/buildingSMART/IDS-Audit-tool)自動的に生成されたものです。
