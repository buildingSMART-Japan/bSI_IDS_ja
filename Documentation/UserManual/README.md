...
## はじめに
IDSは、情報**仕様の**リストを含む`.ids`終わるファイル形式である。例えば、1つの**仕様**書には、"*すべての壁に耐火等級プロパティが必要で*"あると記載されている場合があります。IDSファイルを受け取ったモデル作成者は、**各仕様**書に必要な情報がすべて提供されていることを確認するために、IDSファイルを使用することができます。モデルの受信者は、IDSファイルを使用して、IFCモデルがすべての**仕様を満たして**いるかどうかを確認できます。また、**仕様**準拠チェックの結果を一覧にしたレポートを作成することもできます。
