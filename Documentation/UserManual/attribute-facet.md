...
命名規則に従い、要素を正確に記述することは、AECO業界の多くのユースケースにとって非常に重要です。 このため、AECO業界では一般的に**アトリビュートファセット**の両方で**適用性**そして**必要条件**セクション**仕様**.
...
| **価値** | ❌ | ✔️ | 属性のデータ型に適した任意の値 | 属性の値は一致しなければならない。詳しくは[DataType documentation](../ImplementersDocumentation/DataTypes.md#xml-base-types)を参照。 |
...
| DurationType が WORKTIME に設定された (つまりカレンダーに基づいた) エンティティ (通常は IfcTaskTime) | 期間タイプ（例えばIfcTaskTimeの場合）は、経過時間ではなく、カレンダーに基づいていなければならない。 | Name="DurationType", Value="WORKTIME" |
