...
IFCで定義されている型の全リストは、IFCのドキュメントに記載されている。

| Ifc定義型名 | 物理量の説明 | 単位 | 単位記号 | デフォルト表示 | 次元指数 | 単位列挙 | IfcSIUnitName列挙型 |
| --------------------------------------------- | --------------------------------------- | ------------ | ----------- | --------------- | ----------------------- | ---------------------------------------------------------- | -------------------------------------------------- |
...
| 物質量測定値 | 物質量 | モル | mol | mol | (0, 0, 0, 0, 0, 1, 0) | IfcUnitEnum.AMOUNTOFSUBSTANCEUNIT。 | ifcSIUnitName.MOLE |
...
| 曲率測定 | 曲率 |  |  | rad / m | (-1, 0, 0, 0, 0, 0, 0) | IfcDerivedUnitEnum.CURVATUREUNIT。 |  |
| 等価測定 | 投与量に相当する | シーベルト | Sv | J / kg | (2, 0, -2, 0, 0, 0, 0) | IfcUnitEnum.DOSEEQUIVALENTUNIT | ifcSIUnitName.SIEVERT |
| 動粘度測定法 | 動的粘度 |  |  | Pa s | (-1, 1, -1, 0, 0, 0, 0) | ifcDerivedUnitEnum.DYNAMICVISCOSITYUNIT |  |
...
| 磁束測定 | 磁束 | ウェーバー | Wb | Wb | (2, 1, -2, -1, 0, 0, 0) | IfcDerivedUnitEnum.MAGNETICFLUXUNIT。 | IfcSIユニット名.WEBER |
...
| ifc平面力測定 | 平面力 | パスカル | Pa | Pa | (-1, 1, -2, 0, 0, 0, 0) | IfcDerivedUnitEnum.PLANARFORCEUNIT。 | ifcSIUnitName.PASCAL |
...
| IFC正平面角測定法 | 正の平面角 | ラジアン | rad | rad | (0, 0, 0, 0, 0, 0, 0) | IfcUnitEnum.PLANEANGLEUNIT | IfcSIUnitName.RADIAN |
| IFCパワーメジャー | パワー | ワット | W | W | (2, 1, -3, 0, 0, 0, 0) | IfcUnitEnum.POWERUNIT | IfcSIUnitName.WATT |
| 圧力測定 | 圧力 | パスカル | Pa | Pa | (-1, 1, -2, 0, 0, 0, 0) | IfcUnitEnum.PRESSUREUNIT | ifcSIUnitName.PASCAL |
...
| 音圧レベル測定 | 音圧レベル（対数基準 2e-5 Pa） | デシベルSPL | db | db | (0, 0, 0, 0, 0, 0, 0) | ifcDerivedUnitEnum.SOUNDPRESSURELEVELUNIT |  |
| 音圧測定 | 音圧 | パスカル | Pa | Pa | (-1, 1, -2, 0, 0, 0, 0) | IfcDerivedUnitEnum.SOUNDPRESSUREUNIT |  |
...
## 寸法単位
次元指数は、ISO 80000-1 で定義されている[国際量体系に従って](https://en.wikipedia.org/wiki/International_System_of_Quantities)指定される。その順番は、長さ、質量、時間、電流、熱力学的温度、物質量、光度（m、kg、s、A、K、mol、cd）である。例えば、速度（m/s）は*（1, 0, -1, 0, 0, 0, 0*）と表記される。

## 変換例
ソフトウェアでは通常、値はローカル単位で表示される。次の表は、ユーザーに対して物事がどのように表現され、IDSではどのように表現されるかの例をいくつか挙げたものである。
